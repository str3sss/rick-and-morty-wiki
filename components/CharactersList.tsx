...
export function CharactersList({ data }: Props) {
  ...
  return (
    <div className="flex flex-wrap justify-center mt-8">
      {data.map((elem) => (
        <article key={elem.id} className="bg-primary  text-white mt-4 mx-0 h-60 sm:w-[500px] sm:m-2 gw-11/12 rounded-2xl flex ">
          ...
          <div className="flex flex-[3_1_0%] pt-4 flex-col relative bg-primary rounded-r-xl">
            ...
            <span className="px-6">
              ...
              <span className="h-3 w-3 left-2 relative">
                <span className={statusStyle(elem.status, 'animate-ping inline-flex h-3 w-3 absolute top-1 rounded-full')}></span>
